...
class DocMasterInstaller:
    ...
    def ensure_icon_exists(self):
        """Garante que o arquivo de ícone exista"""
        icon_dir = self.base_dir / "static" / "images"
        ...
        if not icon_file.exists():
            ...
            # Tenta criar um ícone básico usando PIL
            try:
                from PIL import Image, ImageDraw
                
                # Cria uma imagem quadrada de 256x256 pixels
                img = Image.new('RGB', (256, 256), color=(37, 99, 235))  # Cor primária azul
                
                # Adiciona um desenho simples
                ...
                draw.rectangle(
                    [(70, 70), (186, 186)],
                    fill=(16, 185, 129)  # Cor secundária verde
                )
                
                # Salva como .ico
                img.save(str(icon_file), format='ICO')
                logging.info(f"Ícone criado em {icon_file} ✓")
                return True
            except Exception as e:
                logging.warning(f"Não foi possível criar o ícone: {e}")
                return False
    ...
    def create_executable(self):
        ...
        # Verifica se o arquivo .env existe, caso contrário, cria a partir do .env.example
        if not self.env_file.exists() and self.env_example.exists():
            logging.info("Arquivo .env não encontrado. Criando a partir do .env.example...")
            shutil.copy(self.env_example, self.env_file)
        
        # Garante que o ícone exista
        self.ensure_icon_exists()
        
        # Lista de dados para incluir
        datas = []
        
        # Adiciona diretórios que sempre existem
        ...
        # Cria o conteúdo do spec file dinamicamente
        datas_str = ',\n        '.join([f"('{src}', '{dst}')" for src, dst in datas])
        
        # Verifica se o ícone existe
        icon_path = self.base_dir / "static" / "images" / "icon.ico"
        icon_statement = f"icon='{icon_path.relative_to(self.base_dir)}'" if icon_path.exists() else "icon=None"
...
"""
    ...
    def install_and_run(self):
        ...
        try:
            ...
            self.ensure_icon_exists()  # Garante que o ícone exista
